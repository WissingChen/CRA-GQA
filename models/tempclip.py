...
import torch
import math
from modules.m_tempclip.language_model import Bert, LanModel
import copy
from transformers.modeling_outputs import BaseModelOutput
from transformers import BertConfig
from transformers import RobertaConfig
from transformers import DebertaConfig
from utils.misc.misc import get_mask
import torch.nn.functional as F

import h5py
import os.path as osp
from modules.m_tempclip.transformer import Transformer, Embeddings
...
class TempCLIP(nn.Module):
    def __init__(
        ...
        self.linear_video = nn.Linear(cfgs["model"]["feature_dim"], d_model)
        self.norm_video = nn.LayerNorm(d_model, eps=1e-12)

        #####################clip position###################
        self.position_v = Embeddings(d_model, 0, self.max_feats, cfgs["model"]["dropout"], True, d_pos)
        ...
        if cfgs["model"]["lan"] == 'BERT':
            ...
            pt_config_name = '/data2/chenweixing/Other/VQA/roberta-base'
        ...
        self.config = config.from_pretrained(
            pt_config_name,
            num_hidden_layers=cfgs["model"]["num_layers"],
            hidden_size=d_model,
            attention_probs_dropout_prob=cfgs["model"]["dropout"],
            intermediate_size=cfgs["model"]["d_ff"],
            num_attention_heads=cfgs["model"]["num_heads"],
        )
        ...
        if cfgs["model"]["baseline"] in ['NG+', 'NG']:
            self.prop_num = cfgs["model"]["prop_num"]
            self.sigma = cfgs["model"]["sigma"]
    ...
    def forward(self, video, video_mask, question=None, question_mask=None, 
        ...
        answer_g, answer_w = (
            self.get_answer_embedding(answer.cuda())
        ...
        if self.baseline == 'posthoc':
            ...
            props_feat = video_fp.unsqueeze(1) \
                .expand(bsize, self.prop_num, -1, -1).contiguous().view(bsize*self.prop_num, self.max_feats, -1)
        ...
        if fusion_proj is not None and answer_g.device != fusion_proj.device:
            ...
            # fusion_proj [B, N], answer_g [B, 5, N], out [B, 32, 1]
